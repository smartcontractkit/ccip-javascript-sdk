import { address, getAddressEncoder, getAddressDecoder, getAddressFromPublicKey } from '@solana/kit'
...
export function encodeSVMExtraArgsV1({
    ...
    const tokenReceiverBytes = tokenReceiver 
        ? encoder.encode(tokenReceiver)
        : new Uint8Array(32) // Default/zero address
...
export function decodeSVMExtraArgsV1(data: Uint8Array): SVMExtraArgsV1 {
    // Check minimum length (4 bytes tag + minimum data)
    if (data.length < 4 + 4 + 8 + 1 + 32 + 4) { // tag + computeUnits + bitmap + bool + tokenReceiver + accounts length
        throw new Error('Invalid SVM Extra Args V1: data too short')
    }

    // Verify tag
    const tag = new DataView(data.buffer, data.byteOffset, 4).getUint32(0, false) // big endian
    if (tag !== SVM_EXTRA_ARGS_V1_TAG) {
        throw new Error(`Invalid SVM Extra Args V1 tag: expected 0x${SVM_EXTRA_ARGS_V1_TAG.toString(16)}, got 0x${tag.toString(16)}`)
    }
